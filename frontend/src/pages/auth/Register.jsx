...
export default function Register() {
  ...
  const handleRegister = async () => {
    setError(null)
    if (!full_name || !email || !password || !confirmPassword || !departement_id) {
    ...
    setLoading(true)
    const data = await authService.register(full_name, email, password, 6, parseInt(departement_id))
    setLoading(false)
      ...
      <div style={styles.right}>
        <div style={styles.card}>
          ...
          <div style={styles.field}>
            ...
            <input type="text" placeholder="Prénom Nom" value={full_name}
              onChange={e => setFullName(e.target.value)} style={styles.input} />
          </div>

          <div style={styles.field}>
            <label style={styles.label}>Adresse email</label>
            <input type="email" placeholder="prenom.nom@iutv.univ-paris13.fr" value={email}
          ...
          <div style={styles.row}>
            <div style={{ ...styles.field, flex: 1 }}>
              ...
              <input type="password" placeholder="8 caractères min" value={password}
          ...
          <div style={styles.field}>
            ...
            <select value={departement_id} onChange={e => setDepartementId(e.target.value)}
              style={styles.select}>
          ...
          <div style={styles.infoBox}>
            Votre compte sera créé avec un accès <strong>lecteur</strong>. Un administrateur pourra modifier votre rôle si nécessaire.
          </div>
...
const styles = {
  ...
  left: {
    ...
    position: "relative", display: "flex", alignItems: "flex-end", padding: "3rem",
  },
  overlay: { position: "absolute", inset: 0, background: "rgba(5,40,80,0.35)" },
  leftContent: { position: "relative", zIndex: 1 },
  tagline: { color: "rgba(255,255,255,0.92)", fontSize: "1.5rem", fontWeight: 300, lineHeight: 1.5, margin: 0 },
  ...
  title: { fontSize: "1.4rem", fontWeight: 700, color: "#0d2a4a", margin: "0 0 0.25rem" },
  subtitle: { fontSize: "0.8125rem", color: "#6b7a99", margin: "0 0 1.5rem" },
  ...
  infoBox: { background: "#f0f7ff", border: "1px solid #bdd9f5", borderRadius: "8px", padding: "0.75rem 1rem", fontSize: "0.8125rem", color: "#1a5a99", marginBottom: "1.25rem" },
  field: { marginBottom: "1rem" },
  row: { display: "flex", gap: "0.75rem" },
  label: { display: "block", fontSize: "0.8125rem", fontWeight: 600, color: "#3d4f6e", marginBottom: "0.35rem", letterSpacing: "0.02em" },
  input: { width: "100%", padding: "0.65rem 0.875rem", fontSize: "0.9rem", border: "1.5px solid #dde3ec", borderRadius: "8px", outline: "none", boxSizing: "border-box", color: "#0d2a4a", background: "#fff" },
  select: { width: "100%", padding: "0.65rem 0.875rem", fontSize: "0.9rem", border: "1.5px solid #dde3ec", borderRadius: "8px", outline: "none", boxSizing: "border-box", color: "#0d2a4a", background: "#fff", cursor: "pointer" },
  btn: { width: "100%", padding: "0.75rem", fontSize: "0.9375rem", fontWeight: 600, background: "linear-gradient(135deg, #1a7fd4, #0d5fa8)", color: "#fff", border: "none", borderRadius: "8px", cursor: "pointer", letterSpacing: "0.02em" },
  loginLink: { textAlign: "center", fontSize: "0.8125rem", color: "#6b7a99", marginTop: "1.25rem", marginBottom: 0 },
